...
const mockChainConfigService = {
    getInstance: mock(() => ({
        ...
        getSupportedChains: mock(() => ['ethereum', 'arbitrum', 'polygon'])
    }))
};

// Mock the adapter
const mockAdapter = {
    getLatestBlock: mock(() => Promise.resolve({
        blockNumber: 19123456,
        blockTimestamp: 1752762620
    })),
    getPair: mock(() => Promise.resolve({
        id: 'test-pair-id',
        dexKey: 'balancer-v3',
        feeBps: 50,
        asset0Id: '0x123',
...
mock.module('../utils/blockchain-client', () => ({
    getTokenInfo: mock(() => Promise.resolve({
        name: 'Test Token',
        symbol: 'TEST',
        decimals: 18
    }))
...
describe('API Routes - integration tests', () => {
    ...
    describe('Chain Management', () => {
        it('should list supported chains', async () => {
            ...
            expect(response.body.chains).toEqual(['ethereum', 'arbitrum', 'polygon']);
        });
    });
    ...
    describe('Latest Block Endpoint', () => {
        ...
        it('should return 400 for unsupported chain', async () => {
            const response = await request(app)
                .get('/api/unsupported/latest-block')
                .expect(400);
    ...
    describe('Asset Endpoint', () => {
        it('should return asset information', async () => {
            const response = await request(app)
                .get('/api/ethereum/asset')
                .query({ id: '0x123456789' })
                .expect(200);
    ...
    describe('Pair Endpoint', () => {
        it('should return pair information', async () => {
            const response = await request(app)
                .get('/api/ethereum/pair')
                .query({ id: 'test-pair-id' })
                .expect(200);
